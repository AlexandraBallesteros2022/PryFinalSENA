...
const getOneEmpleado = async (req, res) => {
   ...
   try {
      const { result } = await empModel.getOneEmpleado(params.idDocente); // almacenamos el resultado por si queremos trabajar con el mismo
      return res.status(200).send(result);
   } catch (err) {
      console.log(err);
      return res.status(500).send("error");
...
const getEmpleadoByCodigoDocente = async (req, res) => {
   ...
   try {
      const { result } = await empModel.getEmpleadoByCodigoDocente(
         codigoDocente
...
const updateOneEmpleado = async (req, res) => {
   ...
   try {
      ...
      return res.status(200).send("¡Empleado Actualizado exitosamente!");
   } catch (err) {
      console.log(err);
      return res.status(500).send("error");
   }
